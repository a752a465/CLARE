#################################################################################################################
### Title: Adjusted EWAS Using Compositional Data Transformations - Supplementary Simulation #6 ----
...
#################################################################################################################
# QUICK FIGURES ----
## PLOT 1: Mean Power & Type I Error Rate with SD Ribbons ----
colors <- c("#15bf3a",
            "#4075d6",
            "#631307")
shapes <- c(15,1,4)
...
df = Results_Final_Sim1 %>% 
  ...
  mutate(Scenario_2= case_when(Scenario=="DMP Correlated"~"cDMP",
                               Scenario=="Non-DMP Correlated"~"ncDMP"
                               ,TRUE~NA))%>%
  dplyr::filter(Method != "Unadjusted" & !is.na(Scenario_2) & Metric == "Detection")%>%
  group_by(Effect,Sample.Size,Scenario_2,Method)%>%
  summarise(mean=mean(value,na.rm=TRUE),
            sd=sd(value,na.rm=TRUE))%>%
  ungroup()
#
title= "Supplementary Simulation #6: Uncorrelated Cell Mixture"
plot1 <- ggplot(df,aes(x=Sample.Size,y=mean,color=Method,shape=Method))+
  ...
  labs(title=title,
       subtitle = "Power and Type I Error Rates",
       x="Sample Size",
       y="Detection Rate")+
...
## Supplementary plot 1: Mean Bias with SD Ribbons ----
colors <- c("#15bf3a",
            "#4075d6",
            "#631307")
shapes <- c(15,1,4)
...
df_bias = Results_Final_Sim1 %>%
  ...
  mutate(Scenario_2= case_when(Scenario=="DMP Correlated"~"DMP",
                               ...
                               ,TRUE~NA))%>%
  dplyr::filter(!is.na(Scenario_2) & Metric == "Bias"
                #& Method != "Unadjusted"
  )%>%
  group_by(Effect,Sample.Size,Scenario_2,Method)%>%
  summarise(mean=mean(value,na.rm=TRUE),
  ...
  ungroup()
#
title = "Simulation 1: Mean Bias in Estimated Effect"
plot2 <- ggplot(df_bias,aes(x=Sample.Size,y=mean,color=Method,shape=Method))+
  geom_line(size=1,alpha=0.5)+
  geom_point(size=2,alpha=0.9)+
  geom_ribbon(aes(ymin=mean-sd,ymax=mean+sd,fill=Method),alpha=0.1,color=NA)+
  labs(title=title,
  ...
  scale_color_manual(values=colors)+
  scale_shape_manual(values=shapes)+
  facet_grid(Scenario_2~Effect,scales="free_y")+
...
## Supplementary plot 2: Mean RMSE with SD Ribbons ----
unique(Results_Final_Sim1$Effect)

df_rmse = Results_Final_Sim1 %>%
  mutate(Method=factor(Method,levels=c("CLARE","Proportion","Unadjusted")))%>%
  mutate(Effect=case_when(Effect=="Type I Error"~"No Effect",
                          Effect=="Power"~"True Effect",TRUE~NA))%>%
  mutate(Effect=factor(Effect,levels=c("No Effect","True Effect")))%>%
  mutate(Scenario_2= case_when(Scenario=="DMP Correlated"~"DMP",
                               Scenario=="Non-DMP Correlated"~"Non-DMP"
                               ,TRUE~NA))%>%
  dplyr::filter(!is.na(Scenario_2) & Metric == "RMSE"
                #& Method != "Unadjusted"
  )%>%
  group_by(Effect,Sample.Size,Scenario_2,Method)%>%
  summarise(mean=mean(value,na.rm=TRUE),
  ...
  ungroup()
#
title = "Simulation 1: Mean RMSE of Estimated Effect"
plot3 <- ggplot(df_rmse,aes(x=Sample.Size,y=mean,color=Method,shape=Method))+
  geom_line(size=1,alpha=0.5)+
  geom_point(size=2,alpha=0.9)+
...
## Supplementary plot 3: Mean Variance with SD Ribbons ----
df_rmse = Results_Final_Sim1 %>%
  mutate(Method=factor(Method,levels=c("CLARE","Proportion","Unadjusted")))%>%
  mutate(Effect=case_when(Effect=="Type I Error"~"No Effect",
                          Effect=="Power"~"True Effect",TRUE~NA))%>%
  ...
  mutate(Scenario_2= case_when(Scenario=="DMP Correlated"~"DMP",
                               Scenario=="Non-DMP Correlated"~"Non-DMP"
                               ,TRUE~NA))%>%
  dplyr::filter(!is.na(Scenario_2) & Metric == "Variance"
                #& Method != "Unadjusted"
  )%>%
  group_by(Effect,Sample.Size,Scenario_2,Method)%>%
  summarise(mean=mean(value,na.rm=TRUE),
  ...
  ungroup()
#
title = "Simulation 1: Mean Variance of Estimated Effect"
plot4 <- ggplot(df_rmse,aes(x=Sample.Size,y=mean,color=Method,shape=Method))+
  geom_line(size=1,alpha=0.5)+
  geom_point(size=2,alpha=0.9)+
  geom_ribbon(aes(ymin=pmax(mean-sd,0),ymax=mean+sd,fill=Method),alpha=0.1,color=NA)+
  labs(title=title,
