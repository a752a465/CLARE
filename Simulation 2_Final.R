#################################################################################################################
### Title: Adjusted EWAS Using Compositional Data Transformations - SIMULATION 2 ----
...
### DA Cells Simulation Parameters ----

#### 2 Cell Type DA (Indirect Effect: 0.03820667) ----
...
lambda1=lambda1*rho
#
Cell_2_parameters <- list("Lambda0"=lambda0,"Lambda1"=lambda1,"B0"=B0)


#### 3 Cell Type DA (Indirect Effect: 0.03822571) ----
lambda0=c("Neut"=50,"Mono"=5,"Eos"=3,"Baso"=1,"CD4_Naive"=12,"CD4_Mem"=7,"CD8_Naive"=3,"CD8_Mem"=4,"B_Naive"=4,"B_Mem"=3,"Treg"=2,"NK"=6)
tau = c("Neut"=-10,"Mono"=0,"Eos"=0,"Baso"=0,"CD4_Naive"=+3,"CD4_Mem"=0,"CD8_Naive"=+2,"CD8_Mem"=0,"B_Naive"=0,"B_Mem"=0,"Treg"=0,"NK"=0)
B0= c("Neut"=0.8,"Mono"=0.518,"Eos"=0.236,"Baso"=0.647,"CD4_Naive"=0.2,"CD4_Mem"=0.383,"CD8_Naive"=0.199,"CD8_Mem"=0.811,"B_Naive"=0.236,"B_Mem"=0.405,"Treg"=0.429,"NK"=0.647)
rho=total.cells/sum(lambda0)
lambda1 = lambda0+tau
lambda0=lambda0*rho
lambda1=lambda1*rho
#
Cell_3_parameters <- list("Lambda0"=lambda0,"Lambda1"=lambda1,"B0"=B0)


#### 4 Cell Type DA (Indirect Effect: 0.03843524) ----
lambda0=c("Neut"=50,"Mono"=5,"Eos"=3,"Baso"=1,"CD4_Naive"=12,"CD4_Mem"=7,"CD8_Naive"=3,"CD8_Mem"=4,"B_Naive"=4,"B_Mem"=3,"Treg"=2,"NK"=6)
...
#################################################################################################################
# SIMULATIONS ----
## 1. 2 Cell Type DA (Indirect Effect: 0.03820667) -----
...
Scenario_params = Cell_2_parameters
...
# Packing
Results_Cell_2 = Sim_Results
rm(Sim_Results,Scenario_params)

## 2. 3 Cell Type DA (Indirect Effect: 0.03822571) -----
set.seed(999848)
# Prepare Results Dataframe & Scenario Parameters
Sim_Results = data.frame("Effect"=NA,"Sample Size"=NA,"CLARE"=NA,"Proportion"=NA,"Unadjusted"=NA)
Scenario_params = Cell_3_parameters
...
# Packing
Results_Cell_3 = Sim_Results
rm(Sim_Results,Scenario_params)

## 3. 4 Cell Type DA (Indirect Effect: 0.03843524) ----
set.seed(999848)
# Prepare Results Dataframe & Scenario Parameters
Sim_Results = data.frame("Effect"=NA,"Sample Size"=NA,"CLARE"=NA,"Proportion"=NA,"Unadjusted"=NA)
...
# Packing
Results_Cell_4 = Sim_Results
rm(Sim_Results,Scenario_params)

## 5. Packaging & Saving ----

# Notes for Individual Scenarios
Results_Cell_2$Scenario = "2 DA Cells"
Results_Cell_3$Scenario = "3 DA Cells"
Results_Cell_4$Scenario = "4 DA Cells"

# Complete Data Frame
Results_Final_Sim2 = bind_rows(Results_Cell_2,Results_Cell_3,Results_Cell_4)%>%
...
shapes <- c(15,1,4)
#
sample.size.breaks=unique(Results_Final_Sim2$Sample.Size)

df = Results_Final_Sim2 %>% 
...
## PLOT 2: Mean Power & FDR with CIs ----
colors <- c("#15bf3a",
            "#4075d6",
            "#631307")
shapes <- c(15,1,4)
...
# EXPORTING RESULTS ----
if(Outputs==TRUE){
  filename <- "Simulation 2.xlsx"
  filename <- paste0(Parent_Directory,"Results/",filename)
  data_list <- list(Data=Results_Final_Sim2)
